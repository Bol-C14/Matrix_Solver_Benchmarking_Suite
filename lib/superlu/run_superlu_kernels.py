...
class SuperluBenchmark:
    def __init__(self, engine, database_folder, timeout=100):
        """
        Initialize the SuperluBenchmark class with the engine path, database folder, and timeout.
        """
        self.engine = engine
    ...
    def run_single(self, engine, nrhs, filename, bmatrix, reps):
        """
        Run a single benchmark command for superlu on the given matrix file.
        """
        try:
    ...
    def run_benchmark(self):
        ...
        for i in tqdm(range(len(self.mtxs)), desc="Processing Matrices"):
            ...
            try:
                ...
                self.df.loc[i] = [i, self.mtxs[i], 'SUPERLU', 1, nnz, num_rows, t1, t2]
        ...
        self.df.to_csv('results_superlu_kernel.csv', index=False)
        logging.info('Results saved to results_superlu_kernel.csv')
        print('done!')

# Example usage of the class
if __name__ == "__main__":
    engine = './superlu_kernel'
    database_folder = "/mnt/c/Work/transient-simulation/Random_Circuit_Generator/random_circuit_matrixs"

    superlu_benchmark = SuperluBenchmark(engine, database_folder)

    ...
    for j in range(len(threads)):
        for i, mtx in enumerate(mtxs):
            ...
            try:
                ...
                df.loc[loc] = [loc, mtx, 'PARDISO', threads[j], nnz, num_rows, t1, t2]
                logging.debug(f"Stored results for matrix: {mtx}")
                logging.info(f"PARDISO - Processed {loc + 1}/{len(mtxs) * len(threads)} matrices.")
                loc += 1
            except Exception as e:
                logging.info(f"Error processing matrix {filepath}: {e}")

    return df

def pardiso_solve(database_folder=default_database_folder, engine=default_engine, timeout=default_timeout, debug_info=1, threads=[1, 4, 8, 12, 24, 32]):
    """
    Solve the circuit matrix using the PARDISO algorithm.
